  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ ...values, source: "website-contact" }),
      });
  ...
  return (
    <form className="contact-form" onSubmit={submit} ref={formRef} noValidate>
      ...
      <div className="field-grid">
        ...
        <label className="field-full"><span>Message *</span><textarea name="message" rows={5} required placeholder="Tell us about the vessel, current setup, and what you want to improve." /></label>
      </div>
