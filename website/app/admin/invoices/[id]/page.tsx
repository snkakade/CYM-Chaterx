...
import { CharterXWordmark } from "@/components/CharterXWordmark";
...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="invoice-page admin-page">
      <div className="invoice-toolbar"><a href="/admin">Back to operations</a><PrintButton /></div>
      <article className="invoice-document">
        ...
        <section className="invoice-meta">
          <div><span>Issued by</span><strong>Collaborative Yacht Management Limited</strong><p>Trading as CharterX<br />United Kingdom<br />connect@cymcharterx.com</p></div>
        ...
        <div className="invoice-summary"><p><span>Subtotal</span><strong>{money(invoice.subtotal_cents, invoice.currency)}</strong></p><p><span>Tax / VAT ({(invoice.tax_rate_bps / 100).toFixed(2)}%)</span><strong>{money(invoice.tax_cents, invoice.currency)}</strong></p><p><span>Total due</span><strong>{money(invoice.total_cents, invoice.currency)}</strong></p></div>
        {invoice.notes && <section className="invoice-notes"><span>Notes &amp; payment instructions</span><p>{invoice.notes}</p></section>}
        <footer><p>Clear systems. Stronger bookings.</p><span>CYM CharterX · {invoice.invoice_number}</span></footer>
      </article>
    </div>
  );
}
